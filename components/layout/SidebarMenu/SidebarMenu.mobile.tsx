...
export default function SidebarMenuMobile({ isOpen, onClose, id }: SidebarMenuProps) {
  ...
  return (
    <>
      ...
      <div 
        id={id}
        className={`fixed top-0 right-0 h-full w-80 bg-white dark:bg-gray-900 shadow-2xl transform transition-transform duration-300 z-50 flex flex-col ${
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-500 dark:text-gray-400 text-sm">Islamic Knowledge</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {activeTab === 'menu' && (
            <div className="p-4">
              {/* Quick Actions */}
              <div className="mb-6">
                <h3 className="font-semibold text-gray-900 dark:text-white mb-3 text-sm">Quick Access</h3>
                <div className="grid grid-cols-2 gap-2">
                  {quickActions.map((action) => (
                    <Link
                      key={action.name}
                      href={action.href}
                      className={`p-3 rounded-xl bg-linear-to-br ${action.color} text-white text-center transition-all duration-200 hover:scale-105 hover:shadow-lg`}
                    >
